...
# Cache
# Shared Redis cache in production so state that must be consistent across
# gunicorn workers (DRF throttle counters in particular) is not held in
# per-process memory. Falls back to local memory when REDIS_URL is unset.
